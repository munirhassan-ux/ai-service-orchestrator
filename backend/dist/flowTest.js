...
async function runFlowTest() {
    ...
    const { booking } = createBooking(intent, selectedProvider, quote, finalPrice, mockThreadId, "customer_001");
    ...
    const origP = origProviders.find((p) => p.provider_id === selectedProvider.provider_id);
    ...
    const updatedP = updatedProviders.find((p) => p.provider_id === selectedProvider.provider_id);
    ...
    const finalP = finalProviders.find((p) => p.provider_id === selectedProvider.provider_id);
...
//# sourceMappingURL=flowTest.js.map