...
export function calculatePrice(intent, provider, allProviders, userJobCount = 0) {
    ...
    const distance_fee = provider.distance_km > 3
        ...
        : 0;
    // Urgency surcharge
    const urgency_surcharge = intent.urgency === "high" ? 200 :
        intent.urgency === "medium" ? 100 : 0;
    // Complexity premium
    const complexity_premium = intent.job_complexity_hint === "complex" ? 200 :
    ...
    const min_rate_total = roundToNearest10((visit_fee + minimum_price_per_hour * hours_min + extras) * surge_multiplier);
