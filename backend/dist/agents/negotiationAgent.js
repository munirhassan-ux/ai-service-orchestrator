...
export function createNegotiationThread(bookingRequestId, providerId, customerId, aiQuote, sessionId, preferredTime) {
...
export async function providerRespond(threadId, action, counterPrice, reason) {
    const data = readThreads();
    ...
    if (action === "accept") {
        ...
        if (thread.round >= thread.max_rounds) {
            const customerLastOffer = thread.messages.filter((m) => m.from === "customer").slice(-1)[0]?.offered_price ||
                thread.ai_quote;
...
export async function customerRespond(threadId, action, counterPrice) {
    const data = readThreads();
    const thread = data.threads.find((t) => t.id === threadId);
    if (!thread)
        throw new Error(`Thread ${threadId} not found`);
    ...
    if (action === "accept") {
        ...
        if (thread.round >= thread.max_rounds) {
            const providerLastOffer = thread.messages.filter((m) => m.from === "provider").slice(-1)[0]?.offered_price ||
                thread.ai_quote;
...
export function acceptMidpoint(threadId, acceptedBy) {
    const data = readThreads();
    const thread = data.threads.find((t) => t.id === threadId);
    if (!thread)
        throw new Error(`Thread ${threadId} not found`);
    ...
    if (!midpointMsg)
        throw new Error("No AI midpoint found");
    ...
    const bothAccepted = thread.messages.filter((m) => (m.from === "customer" || m.from === "provider") &&
        m.offered_price === midpointMsg.offered_price).length >= 2;
    if (bothAccepted) {
        thread.status = "agreed";
...
export function declineMidpoint(threadId, declinedBy) {
    const data = readThreads();
    const thread = data.threads.find((t) => t.id === threadId);
    if (!thread)
        throw new Error(`Thread ${threadId} not found`);
...
async function generateAIMidpointMessage(thread, midpoint) {
...
//# sourceMappingURL=negotiationAgent.js.map