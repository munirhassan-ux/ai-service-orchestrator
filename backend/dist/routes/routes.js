...
router.get("/booking/:id", (req, res) => {
    ...
    try {
        ...
        providerObj = providers.find((p) => p.provider_id === booking.provider_id);
    }
    catch (err) {
...
router.post("/booking/simulate-step", (req, res) => {
    ...
    try {
        ...
        const bIdx = bookingsData.bookings.findIndex((b) => b.booking_id === booking_id);
        if (bIdx !== -1) {
...
router.post("/booking/submit-rating", (req, res) => {
...
router.post("/booking/cancel-provider", (req, res) => {
    const { booking_id } = req.body;
    if (!booking_id)
...
router.post("/notifications/clear", (req, res) => {
    ...
    try {
        ...
        if (idx !== -1)
            notificationsQueue.splice(idx, 1);
